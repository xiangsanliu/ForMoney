...
public class CommercialHouseSellPresenter extends ListItemPresenter<SellRentModel, List<SellRentModel>, ListItemView<SellRentModel>> {

    private BaseModel model;

    public CommercialHouseSellPresenter(Context context) {
        super(context);
    ...
    protected Observable<List<SellRentModel>> load(int page) {
        return Observable.create(new Observable.OnSubscribe<List<SellRentModel>>() {
            ...
            public void call(Subscriber<? super List<SellRentModel>> subscriber) {
                subscriber.onNext(model.queryCommercialHouseSellTable());
            }
        });
    }
}
